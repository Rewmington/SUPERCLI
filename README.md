...
- 原生终端嵌入：通过 `node-pty` 运行 PowerShell、Codex、Claude Code、Hermes Agent 等 CLI
- 快捷启动：内置 Codex、Claude Code、Hermes Agent、PowerShell、npm、Git，并支持新增自定义启动项
- 启动器命令列表：每类工具带可展开的常用命令子列表，可直接发送到当前终端
- Hermes Agent 命令支持：内置 `hermes` / `hermes chat` 启动入口，以及 `/help`、`/exit`、`/reset`、`/clear`、`/model` 常用命令
...
- `SUPER-CLI-0.4.0-x64.exe`（Windows x64 安装包）
